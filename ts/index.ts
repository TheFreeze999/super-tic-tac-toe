...
const subGridEls = Array.from(document.querySelectorAll(".sub-grid")) as HTMLDivElement[];
const cellElsBySubGrid = subGridEls.map(subGridEl => Array.from(subGridEl.children)) as HTMLDivElement[][];
const allCellEls = cellElsBySubGrid.flat();

const turnEl = document.querySelector(".turn") as HTMLParagraphElement;
		...
		cellEl.addEventListener("click", () => {
			...
			if (subGridNum !== game.lastMovePositionWithinSubGrid[opponent] && game.lastMovePositionWithinSubGrid[opponent] !== null) {
				if (subGrid.result === null)
					if (game.subGrids[game.lastMovePositionWithinSubGrid[opponent] as number].result === null)
						return;
			}
			...
			game.subGrids.forEach((subGrid, i) => {
				...
				if (subGrid.result !== null) subGridEl.classList.add("inactive");
